...
class GPClassifier:
    ...
    def predict(self, X_test):
        ...
        with torch.no_grad():
            observed_pred = self.likelihood(self.model(X_test_tensor))
            pred_labels = observed_pred.mean > 0.5  # Predicted class labels (0 or 1)
            return pred_labels.int().numpy()  # Convert predictions to numpy array of integers

def cross_validate(n_iter, number_samples, scaler_no, kernel_type, n_splits=2):
    # Select scaler
    if scaler_no == 1:
    ...
    for fold, (train_idx, test_idx) in enumerate (kf.split(X_train)):
        ...
        print(f"Fold {fold + 1} predictions: {predictions}")

    # Compute and print average accuracy and standard deviation
